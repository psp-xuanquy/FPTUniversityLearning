...
namespace BSAPP
{
    public partial class PendingOrders : Form
    {
        ...
        private void PendingOrders_Load(object sender, EventArgs e)
        {
            ...
            dtg_ViewOrder.DataSource = orderRepositories.GetPendingOrders(userId).Select(o => new { o.User.FullName, o.Address, o.Phone, o.PaymentMethod.PaymentMethodName, o.Provider.ProviderName, o.Total, o.CreateAt, o.EstimatedDate }).ToList();

        }

        private void btn_CancelNest_Click(object sender, EventArgs e)
        {
            try
            {
                orderRepositories.OrderItemRequestCancellation(dtg_ViewProduct.CurrentRow.Cells[0].Value.ToString());
                dtg_ViewProduct.DataSource = orderRepositories.GetPendingProducts(dtg_ViewOrder.CurrentRow.Cells[0].Value.ToString());
                dtg_ViewProduct.Columns["ProductId"].Visible = false;
            }
            catch
            {
                MessageBox.Show("Cancel Nest Failed.", "Cancel");
            }
        }

        private void btn_CancelBird_Click(object sender, EventArgs e)
        {
            try
            {
                orderRepositories.OrderItemRequestCancellation(dtg_ViewProduct.CurrentRow.Cells[0].Value.ToString());
            ...
            {
                MessageBox.Show("Cancel Bird Failed.", "Cancel");
            }
        }
