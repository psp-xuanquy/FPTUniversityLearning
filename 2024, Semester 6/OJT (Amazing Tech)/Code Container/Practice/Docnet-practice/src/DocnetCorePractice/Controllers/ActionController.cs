    ...
    public class ActionController : ControllerBase
    {
        ...
        public IActionResult GetCaffeById(string id)
        {
            try
            {
                ...
                _logger.Information($"Caffe found: {jsonCaffe}");
                return Ok(jsonCaffe);
            }
            catch (Exception ex)
            {
        ...
        public ActionResult<CaffeEntity> UpdateCaffe([FromBody] CaffeUpdateModel updateModel)
        {
            try
            {
                _logger.Information($"Start updating caffe with Id {updateModel.Id}");

                var existingCaffe = caffes.FirstOrDefault(x => x.Id == updateModel.Id);

                if (existingCaffe == null)
                {
                    return NotFound($"Caffe with Id {updateModel.Id} not found.");
                }

                if (updateModel.Price < 0 || updateModel.Discount < 0)
                {
                    return BadRequest("Price or Discount cannot be negative.");
                }

                existingCaffe.Price = updateModel.Price;
                existingCaffe.Discount = updateModel.Discount;

                _logger.Information($"Caffe updated: {existingCaffe}");

                return Ok(existingCaffe);
            }
            catch (Exception ex)
            {
                _logger.Error(ex, $"Error during updating caffe with Id: {updateModel.Id}");
                return StatusCode(500, "An internal server error occurred. Please check the logs for details.");
            }
        }
        ...
        public IActionResult DeleteCaffe(string id)
        {
            try
            {
                ...
                return Ok(new { Message = $"Caffe with Id {id} deleted successfully" });
            }
            catch (Exception ex)
            {
